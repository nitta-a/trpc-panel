  ...
  return (
    <>
      {path.length > 0 && (
        <button
          type='button'
          className={`font-bold flex flex-row items-center justify-between w-full ${shown ? '' : 'opacity-70'}`}
          onClick={onClick}
        >
          ...
          {node.nodeType === 'router' ? (
            <Chevron className={'ml-2 w-3 h-3 ' + ``} direction={shown ? 'down' : 'right'} />
          ) : (
            <div />
          )}
        </button>
      ...
      {shown && node.nodeType === 'router' && (
        <div className="pl-2 flex flex-col items-start space-y-2 self-stretch">
          {Object.entries(node.children).map(([key, node]) => {
            const path = node.nodeType === 'procedure' ? node.pathFromRootRouter : node.path
            return <SideNavItem path={path} node={node} key={key} />
          })}
        </div>
      )}
    </>
